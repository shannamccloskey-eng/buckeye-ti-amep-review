...
Review the uploaded Residential plan set and provide:
...
def main(embed: bool = False):
    """
    Residential review with session_state persistence,
    ICC button, and Reset button.
    """
    if not embed:
        st.set_page_config(
            page_title="City of Buckeye – Residential Review (Beta)",
            layout="wide",
            initial_sidebar_state="collapsed",
        )

    st.title("City of Buckeye – Residential Review (Beta)")
    st.caption(f"Model: `{MODEL_NAME}`")
    st.write(
        "Upload Residentia plan set PDF(s) to generate an "
    ...
    project_description = st.text_area(
        ...
        placeholder=(
            "Example: Single Home Dwelling, or Standard Plan, no structural changes, "
            "new restrooms and lighting only."
        ),
    )

    run_button = st.button("Run Residential Review", type="primary")
    ...
    if run_button:
        ...
        try:
            ...
            st.error(f"Error during Residential review: {e}")
            progress_bar.empty()
        ...
        st.success("Residential review complete.")
    ...
    if review_text:
        ...
        if st.button("Save Residential Feedback", key="ti_save_feedback"):
        ...
        with col_reset:
            if st.button("Start New Residential Review / Reset Results", key="ti_reset"):
