...
MODEL_NAME = "gpt-5.1"  # or your preferred model
...
def main(embed: bool = False):
    ...
    embed = False  → standalone (this file run directly): sets page_config.
    embed = True   → called from the master app: skips page_config so it
                     doesn't raise the 'set_page_config can only be called
                     once' error.
    """
